"""
HLS Recorder with Per-Ball Threading Architecture + app8.py Precision Sync

✨ UPGRADED WITH APP8.PY PRECISION SYNC FEATURES:
- Frame-accurate PTS logging during recording (showinfo filter)
- SRT subtitle embedding with per-frame timestamps
- Multi-camera synchronization using PTS overlap detection
- Hardware-accelerated encoding (NVENC/QSV/VideoToolbox)
- Professional timecode synchronization
...
# ===================== app8.py Precision Sync Utilities =====================
# PTS extraction regex for showinfo filter output
PTS_RE = re.compile(r"pts_time:\s*([0-9]+(?:\.[0-9]+)?)")

# Target CFR for precision recording (matches app8.py)
TARGET_FPS_WRITE = 90  # 90fps for cricket biomechanics
...
S3_BUCKET = os.getenv("S3_BUCKET_NAME", "cricket-analysis-bucket1")
AWS_REGION = os.getenv("AWS_REGION", "us-east-1")

# Initialize S3 client
def get_s3_client():
    """Get S3 client using aws configure credentials (default boto3 credential chain)"""
    try:
        # Use default credential chain which picks up aws configure credentials
        # This will use credentials from ~/.aws/credentials (or C:\Users\...\.aws\credentials on Windows)
        logger.info("✅ Using AWS credentials from aws configure (default credential chain)")
...
# ===================== app8.py-Style Precision Sync Functions =====================

def _extract_first_last_pts(log_path: Path) -> tuple[Optional[float], Optional[float]]:
    """
    Extract first and last pts_time from FFmpeg showinfo logs.
    This gives frame-level precision for sync calculation (from app8.py).
    """
...
def _secs_to_timecode(seconds: float, fps: int) -> str:
    """Convert seconds → HH:MM:SS:FF for a given integer fps (from app8.py)."""
    if seconds < 0:
        seconds = 0.0
    hh = int(seconds // 3600)
    rem = seconds - hh * 3600
...
def _fmt_srt_time(sec: float) -> str:
    """Format seconds as SRT timestamp HH:MM:SS,mmm (from app8.py)."""
    ms = int(round((sec - int(sec)) * 1000))
    base = time.gmtime(int(sec))
    return f"{base.tm_hour:02}:{base.tm_min:02}:{base.tm_sec:02},{ms:03d}"


def _write_srt_for_range(log_path: Path, srt_out: Path, global_start: float, global_end: float, fps: int) -> bool:
    """
    Build an SRT whose cue times are relative to the global_start (i.e., common zero).
    One cue per frame, with 1-frame-long duration, carrying "t=<seconds>" (from app8.py).
    """
    if not log_path.exists():
        return False
    ...
    with open(srt_out, "w", encoding="utf-8") as srt:
        for idx, t in enumerate(pts_list, start=1):
            ...
            srt.write(f"{idx}\n")
            srt.write(f"{_fmt_srt_time(start_sec)} --> {_fmt_srt_time(end_sec)}\n")
            srt.write(f"t={t:.6f}s\n\n")
    return True


def _mux_subtitle_into_mp4(video_in: Path, srt_in: Path, video_out: Path) -> bool:
    """
    Mux SRT as mov_text into MP4; keep video stream untouched (copy) (from app8.py).
    """
    cmd = [
        _FFMPEG_BIN, "-y",
        "-i", str(video_in),
    ...
    try:
        with open(logp, "w", buffering=1) as lf:
            ...
            if proc.returncode != 0:
                logger.warning(f"⚠️ SRT muxing failed, see {logp}")
                return False
            return True
    except Exception as e:
        logger.error(f"❌ SRT muxing error: {e}")
        return False


def _best_encoder_args() -> list:
    """
    Detect best available hardware encoder (from app8.py).
    Priority: NVENC (NVIDIA) > QSV (Intel) > VideoToolbox (Apple) > libx264 (CPU)
    """
    common_out = [
        "-pix_fmt", "yuv420p",
        "-movflags", "+faststart",
    ...
    try:
        ...
        if "h264_qsv" in result.stdout:
            logger.info("✅ Using Intel QSV hardware encoder")
            return [
                "-c:v", "h264_qsv",
                "-preset", "medium",
                "-global_quality", "23",
    ...
    try:
        ...
        if "h264_videotoolbox" in result.stdout:
            logger.info("✅ Using Apple VideoToolbox hardware encoder")
            return [
    ...
    # Fallback to CPU
    logger.info("ℹ️ Using CPU encoder (libx264)")
    return ["-c:v", "libx264", "-preset", "veryfast", "-crf", "20"] + common_out


def sync_camera_files_to_overlap(ball_dir: Path, camera_ids: List[str], fps: int = TARGET_FPS_WRITE) -> Dict[str, Path]:
    """
    Synchronize multi-camera files using PTS overlap detection (from app8.py).

    1) Extract first/last pts_time from each camera's .log file
    2) Compute global overlap [global_start, global_end]
    3) Trim each MP4 to the overlap with shared timecode (00:00:00:00)
    4) Generate SRT per camera and mux it in as mov_text
    5) Return dict of {camera_id: synced_mp4_path}
    """
    logger.info(f"🔄 Starting multi-camera sync for {len(camera_ids)} cameras")

    info = []
    for camera_id in camera_ids:
        camera_dir = ball_dir / camera_id
        mp4_files = list(camera_dir.glob("*.mp4"))
        log_files = list(camera_dir.glob("*.log"))

        if not mp4_files or not log_files:
            logger.warning(f"⚠️ [{camera_id}] missing MP4 or log file; skipping")
            continue

        mp4 = mp4_files[0]
        log = log_files[0]

        if mp4.stat().st_size < 2000:
            logger.warning(f"⚠️ [{camera_id}] file too small; skipping")
            continue

        t0, t1 = _extract_first_last_pts(log)
        if t0 is None or t1 is None:
            logger.warning(f"⚠️ [{camera_id}] no PTS data found in {log.name}; skipping")
            continue

        info.append((camera_id, mp4, log, t0, t1))
        logger.info(f"📊 [{camera_id}] PTS range: {t0:.3f}s - {t1:.3f}s (duration: {t1-t0:.3f}s)")

    if len(info) < 2:
        logger.warning("⚠️ Not enough cameras with PTS data to sync; returning original files")
        return {camera_id: ball_dir / camera_id / list((ball_dir / camera_id).glob("*.mp4"))[0]
                for camera_id in camera_ids if (ball_dir / camera_id).glob("*.mp4")}
    ...
    if global_end <= global_start:
        logger.error(f"❌ No overlap found: start={global_start:.3f}s, end={global_end:.3f}s")
        return {}

    duration = global_end - global_start
    logger.info(f"✅ Found overlap: {global_start:.3f}s - {global_end:.3f}s (duration: {duration:.3f}s)")

    # Trim and sync each camera
    synced_files = {}
    for camera_id, mp4, log, t0, t1 in info:
        try:
            start_offset = global_start - t0
            trim_duration = duration

            # Output paths
            synced_mp4 = mp4.parent / f"{mp4.stem}_synced.mp4"
            temp_mp4 = mp4.parent / f"{mp4.stem}_temp.mp4"
            srt_file = mp4.parent / f"{mp4.stem}_synced.srt"

            # Step 1: Trim to overlap with re-encode for frame accuracy
            timecode = _secs_to_timecode(0.0, fps)
            enc_args = _best_encoder_args()
            ...
            trim_cmd = [
                ...
                "-filter:v", f"fps={fps},setpts=PTS-STARTPTS",
                "-timecode", timecode,
            ] + enc_args + [str(temp_mp4)]

            logger.info(f"✂️ [{camera_id}] Trimming: offset={start_offset:.3f}s, duration={trim_duration:.3f}s")

            trim_log = mp4.parent / f"{mp4.stem}_trim.log"
            with open(trim_log, "w") as lf:
                lf.write("CMD: " + " ".join(trim_cmd) + "\n")
                result = subprocess.run(trim_cmd, stdout=lf, stderr=lf)
                if result.returncode != 0 or not temp_mp4.exists():
                    raise RuntimeError(f"Trim failed for {camera_id}, see {trim_log}")

            # Step 2: Generate SRT with frame timestamps
            logger.info(f"📝 [{camera_id}] Generating SRT subtitles")
            if _write_srt_for_range(log, srt_file, global_start, global_end, fps):
                # Step 3: Mux SRT into MP4
                logger.info(f"🔗 [{camera_id}] Muxing SRT subtitles")
                if _mux_subtitle_into_mp4(temp_mp4, srt_file, synced_mp4):
                    temp_mp4.unlink()  # Remove temp file
                    synced_files[camera_id] = synced_mp4
                    logger.info(f"✅ [{camera_id}] Synced file created: {synced_mp4.name}")
                else:
                    # Fallback: use temp file without subtitles
                    temp_mp4.rename(synced_mp4)
                    synced_files[camera_id] = synced_mp4
                    logger.warning(f"⚠️ [{camera_id}] Using file without subtitles")
            else:
                # No SRT, just use trimmed file
                temp_mp4.rename(synced_mp4)
                synced_files[camera_id] = synced_mp4
                logger.warning(f"⚠️ [{camera_id}] No SRT generated, using trimmed file")

        except Exception as e:
            logger.error(f"❌ [{camera_id}] Sync failed: {e}")
            continue
        ...
        sync_info = {
            ...
            "cameras": {
                cam_id: {
                    "original_start": t0,
                    "original_end": t1,
                    "trim_offset": global_start - t0,
                    "synced_file": str(synced_files.get(cam_id, ""))
                }
                for cam_id, _, _, t0, t1 in info
            },
            "synced_at": datetime.now().isoformat()
        }

        sync_json = ball_dir / "sync_info.json"
        with open(sync_json, "w") as f:
            json.dump(sync_info, f, indent=2)
        logger.info(f"📄 Sync metadata saved: {sync_json}")
...
class BallThread(threading.Thread):
    ...
    def _record_ball(self):
        """Record ball from live stream with PARALLEL camera start for better sync."""
        try:
            ...
            # STEP 1: Prepare all recording commands (but don't start yet)
            cameras_to_start = []
            ...
            if not cameras_to_start:
                raise Exception("No cameras available to record")

            # STEP 2: Start ALL cameras as fast as possible (parallel, no delays!)
            logger.info(f"🎬 Starting {len(cameras_to_start)} cameras SIMULTANEOUSLY for Ball {self.ball_recording.ball_index}")
            start_time = time.time()

            for cam_info in cameras_to_start:
                camera_id = cam_info['camera_id']
                hls_url = cam_info['hls_url']
                output_file = cam_info['output_file']

                try:
                    # Start FFmpeg recording WITH PTS LOGGING (app8.py-style)
                    log_file = camera_dir / f"ball_{self.ball_recording.ball_index:02d}_{camera_id}.log"

                    logger.info(f"🎬 Recording {camera_id} with PTS logging for frame-accurate sync")
                    proc = _spawn_ffmpeg_record(
                        hls_url,
                        output_file,
                        preserve_timestamps=True,
                        enable_pts_logging=True,  # Enable app8.py-style PTS logging
                        log_file=log_file
                    )

                    # Store immediately (no sleep between cameras!)
                    with self.lock:
                        self.recording_processes[camera_id] = proc
                        self.ball_recording.local_files[camera_id] = str(output_file)

                    logger.info(f"📹 Started {camera_id} (PID: {proc.pid}, log: {log_file.name})")

                except Exception as e:
                    logger.error(f"❌ Failed to start recording for {camera_id}: {e}")
                    continue

            parallel_start_duration = time.time() - start_time
            logger.info(f"⚡ All cameras started in {parallel_start_duration:.3f}s (parallel launch)")

            # STEP 3: Wait briefly, then verify ALL processes started successfully
            time.sleep(0.5)  # Single sleep after all cameras started

            successful_cameras = []
            failed_cameras = []

            with self.lock:
                for camera_id, proc in list(self.recording_processes.items()):
                    if proc.poll() is not None:
                        logger.error(f"❌ FFmpeg process died immediately for {camera_id}")
                        failed_cameras.append(camera_id)
                        del self.recording_processes[camera_id]
                        if camera_id in self.ball_recording.local_files:
                            del self.ball_recording.local_files[camera_id]
                    else:
            ...
            if failed_cameras:
                logger.warning(f"⚠️ Failed to start cameras: {failed_cameras}")
    ...
    def _process_ball(self):
        """Process recorded ball: sync cameras + generate metadata."""
        try:
            ...
            # STEP 1: Multi-camera synchronization (app8.py-style)
            if len(self.ball_recording.local_files) > 1:
                logger.info(f"🔄 Starting multi-camera sync for {len(self.ball_recording.local_files)} cameras")
                try:
                    synced_files = sync_camera_files_to_overlap(
                        ball_dir,
                        list(self.ball_recording.local_files.keys()),
                        fps=TARGET_FPS_WRITE
                    )

                    if synced_files:
                        logger.info(f"✅ Multi-camera sync completed: {len(synced_files)} cameras synced")
                        # Update local files to point to synced versions
                        for camera_id, synced_path in synced_files.items():
                            self.ball_recording.local_files[camera_id] = str(synced_path)
                    else:
                        logger.warning("⚠️ Multi-camera sync produced no results, using original files")

                except Exception as e:
                    logger.error(f"❌ Multi-camera sync failed: {e}")
                    logger.warning("⚠️ Continuing with original (unsynced) files")
            else:
                logger.info("ℹ️ Single camera recording, skipping sync")

            # STEP 2: Generate timestamp metadata for each camera
            for camera_id, file_path in self.ball_recording.local_files.items():
                ...
                timestamp_data = {
                    ...
                    "fps": TARGET_FPS_WRITE,
                    "synced": "_synced" in Path(file_path).name
                }
...
def _spawn_ffmpeg_record(url: str, output_path: Path, preserve_timestamps: bool = True,
                         enable_pts_logging: bool = False, log_file: Optional[Path] = None) -> subprocess.Popen:
    """
    ...
    Args:
        ...
        preserve_timestamps: Copy timestamps from source
        enable_pts_logging: Enable showinfo filter for frame-accurate PTS logging (app8.py-style)
        log_file: Path to log file for PTS data (required if enable_pts_logging=True)
    """

    if enable_pts_logging:
        # app8.py-style: Re-encode with showinfo logging for frame-accurate sync
        logger.info("🎬 Recording with PTS logging enabled (frame-accurate sync)")

        enc_args = _best_encoder_args()
        fps = int(TARGET_FPS_WRITE)

        # settb=AVTB gives stable timebase; showinfo logs PTS; fps enforces CFR; setpts normalizes to 0
        ...
        cmd = [
            ...
            "-fflags", "+genpts",
            "-i", url,
            "-an", "-sn",  # No audio/subs for now (can add later if needed)
            "-filter:v", vf,
        ] + enc_args + [str(output_path)]

    else:
        # Legacy style: Stream copy without logging (faster but no PTS data)
        logger.info("🎬 Recording with stream copy (fast, no PTS logging)")
    ...
    # Open log file if PTS logging enabled
    log_handle = None
    if enable_pts_logging and log_file:
    ...
    # Store log handle in process object for cleanup
    proc._log_handle = log_handle  # type: ignore

    # Start output reader thread (only if not logging to file)
    if not log_handle:
        def read_output():
            try:
...
def _graceful_stop(proc: subprocess.Popen, timeout: float = 10.0) -> None:
    try:
        ...
        # Close log file if it exists (app8.py-style PTS logging)
            ...
            try:
                ...
                if user_id:
                    try:
                        ...
                        import requests
                        analysis_url = "https://52.5.75.51.nip.io:8000/cricket/analyze-stream-recording"
                        analysis_payload = {
...
def trigger_ball_analysis(ui_session_id: str, user_id: Optional[str] = None):
    ...
    try:
        logger.info(f"🎯 TRIGGER ANALYSIS CALLED for session: {ui_session_id}, user_id: {user_id}")

        # Get all balls for this session
        all_statuses = ball_thread_manager.get_all_balls_status(ui_session_id)
        logger.info(f"📊 All ball statuses: {all_statuses}")
        ...
        for ball_index in ready_balls:
            ...
            if status and status.s3_paths:
                ...
                logger.info(f"📦 Ball {ball_index} S3 paths: {status.s3_paths}")
            else:
                logger.warning(f"⚠️ Ball {ball_index} has no S3 paths stored!")
        ...
        # Trigger analysis (this would call your main backend on EC2)
        try:
            import requests
            # Hardcoded EC2 main API URL - this is where the main cricket analysis API runs
            main_api_base = "https://52.5.75.51.nip.io:8000"
            analysis_url = f"{main_api_base}/cricket/analyze-stream-recording-batch"
            logger.info(f"🔗 Analysis URL configured: {analysis_url}")
            analysis_payload = {
            ...
            def trigger_analysis():
                try:
                    logger.info(f"📡 Calling main API for batch analysis: {analysis_url}")
                    # Skip SSL verification only if explicitly disabled (EC2 uses self-signed certs)
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(f"📊 Analysis response: {response.json()}")
                    else:
                        logger.warning(f"⚠️ Analysis request failed: {response.status_code} - {response.text}")
                except Exception as e:
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting HLS Recorder on http://127.0.0.1:{port}")
    print("📡 Will call main API at: https://52.5.75.51.nip.io:8000")
    print("🛑 Press Ctrl+C to stop")

    # Disable access logs to prevent spam - only show errors/warnings
    uvicorn.run(
        app,
        host="127.0.0.1",
        port=port,
