...
# ==================== LED Serial Sync Config ====================
LED_ENABLED = os.environ.get("LED_ENABLED", "1").strip().lower() not in ("0", "false", "no", "off")
LED_BAUD = int(os.environ.get("LED_BAUD", "9600"))
LED_DEFAULT_PORTS = [
    p.strip()
    for p in os.environ.get("LED_DEFAULT_PORTS", "COM14,COM15").split(",")
    if p.strip()
]
LED_START_COMMAND = os.environ.get("LED_START_COMMAND", "START").strip() or "START"
LED_STOP_COMMAND = os.environ.get("LED_STOP_COMMAND", "STOP").strip() or "STOP"
...
class LedSerialController:
    def __init__(self):
        ...
        self.baud = LED_BAUD
        self.default_ports = list(LED_DEFAULT_PORTS)
    ...
    def _event(self, message: str, *, error: str | None = None):
    ...
    def connect(self, port_name: str | None = None):
        if not self.enabled:
            raise RuntimeError("LED serial sync is disabled")
        if not SERIAL_AVAILABLE:
            raise RuntimeError(f"pyserial is unavailable: {SERIAL_IMPORT_ERROR}")

        candidates = [port_name] if port_name else self.default_ports
        candidates = [p for p in candidates if p]
        if not candidates:
            raise RuntimeError("No LED serial port selected")
        ...
        for candidate in candidates:
            try:
                ...
                time.sleep(0.3)
                self._event(f"Connected to {candidate} at {self.baud} baud")
                return self.snapshot()
            except Exception as exc:
                errors.append(f"{candidate}: {exc}")
                self._event(f"Could not connect to {candidate}: {exc}", error=str(exc))

        raise RuntimeError("Could not connect to LED module. " + " | ".join(errors))
    ...
    def disconnect(self):
        with self._lock:
            was_connected = self._connected_locked()
            self._close_locked()
        if was_connected:
    ...
    def send(self, command: str):
        ...
        if not command:
            raise ValueError("LED command cannot be empty")
        if not self.enabled:
            return {"ok": False, "message": "LED serial sync is disabled", "connected": False}
        if not SERIAL_AVAILABLE:
            return {"ok": False, "message": f"pyserial is unavailable: {SERIAL_IMPORT_ERROR}", "connected": False}

        with self._lock:
            if not self._connected_locked():
                msg = "LED module is not connected"
                self._event(msg, error=msg)
                return {"ok": False, "message": msg, "connected": False}
            try:
                payload = (command + "\r\n").encode("utf-8")
                ...
                self.last_error = None
                self._event(f"TX: {command}")
                return {"ok": True, "message": f"Sent {command}", "connected": True}
            except Exception as exc:
                self.last_error = str(exc)
                self._event(f"Write error: {exc}", error=str(exc))
                self._close_locked()
                return {"ok": False, "message": str(exc), "connected": False}

    def start_sequence(self):
        return self.send(LED_START_COMMAND)

    def stop_sequence(self):
        return self.send(LED_STOP_COMMAND)
    ...
    def _reader_loop(self):
        ...
        while not self._reader_stop.is_set():
            ...
            try:
                ...
                self._event(f"Read error: {exc}", error=str(exc))
                with self._lock:
                    self._close_locked()
                return
            if not data:
            ...
            while True:
                ...
                if text:
                    self.last_rx = text
                    self.last_rx_at = datetime.now(timezone.utc).isoformat()
                    self._event(f"RX: {text}")
    ...
    def snapshot(self):
        ...
        return {
            ...
            "serial_error": SERIAL_IMPORT_ERROR,
            "connected": connected,
            "address": port,
            "baud": self.baud,
            "default_ports": list(self.default_ports),
            "start_command": LED_START_COMMAND,
            "stop_command": LED_STOP_COMMAND,
...
led = LedSerialController()
atexit.register(led.disconnect)
...
# ==================== Routes (API LED Serial Sync) ====================
@app.route("/api/led/status", methods=["GET"])
def api_led_status():
    return jsonify(led.snapshot())


@app.route("/api/led/ports", methods=["GET"])
def api_led_ports():
    try:
        return jsonify({"status": "ok", "ports": led.list_ports(), **led.snapshot()})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e), **led.snapshot()}), 500
...
def api_led_connect():
    try:
        ...
        port = (data.get("address") or data.get("port") or "").strip() or None
        snap = led.connect(port)
        return jsonify({"status": "connected", **snap})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e), **led.snapshot()}), 500
...
def api_led_disconnect():
    try:
        snap = led.disconnect()
        return jsonify({"status": "disconnected", **snap})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e), **led.snapshot()}), 500
...
def api_led_send():
    try:
        ...
        result = led.send(command)
        http_status = 200 if result.get("ok") else 400
        return jsonify({"status": "ok" if result.get("ok") else "error", **result, "led": led.snapshot()}), http_status
    except Exception as e:
        return jsonify({"status": "error", "message": str(e), "led": led.snapshot()}), 500


@app.route("/api/led/start", methods=["POST"])
def api_led_start():
    result = led.start_sequence()
    http_status = 200 if result.get("ok") else 400
    return jsonify({"status": "ok" if result.get("ok") else "error", **result, "led": led.snapshot()}), http_status
