...
class InsoleDevice:
    ...
    async def wait_for_data(self, timeout_s: float = 1.2, poll_s: float = 0.05) -> bool:
        ...
        while time.time() < end:
            if self.raw_notify_count > 0 or self.packet_count > 0:
                return True
    ...
    def _handle_adc_data(self, sender, data):
        ...
        with self._buffer_lock:
            for i in range(packet_count):
                ...
                self.channel_data[0] = struct.unpack_from("<H", packet, 2)[0]
                self.channel_data[1] = struct.unpack_from("<H", packet, 4)[0]
                self.channel_data[2] = struct.unpack_from("<H", packet, 6)[0]
                self.channel_data[3] = struct.unpack_from("<H", packet, 8)[0]
                self.channel_data[4] = struct.unpack_from("<H", packet, 10)[0]
                self.channel_data[5] = struct.unpack_from("<H", packet, 12)[0]
                self.channel_data[6] = struct.unpack_from("<H", packet, 14)[0]
                self.channel_data[7] = struct.unpack_from("<H", packet, 16)[0]
                self._raw_buffer.extend(struct.pack("<d", now))
...
class BleCoordinator:
    ...
    def _active_stream_count(self) -> int:
        ...
        return sum(
            ...
            if d.connected and (d.raw_notify_count > 0 or d.packet_count > 0)
        )
    ...
    async def _scan(self):
        ...
        for d in devices:
            if d.name:
                result.append({"name": d.name, "address": d.address, "rssi": getattr(d, "rssi", None)})
        return result
    ...
    async def _connect_all(self):
        # Connect sequentially; many BLE adapters are more reliable this way than fully parallel connects.
        results = []
        for d in self.devices.values():
    ...
    async def _start_stream(self):
        ...
        for d in self._stream_targets():
            # Do not trust status-bit streaming alone; always (re)arm ADC notify like app13.
            await d.start_stream(freq_cmd=self.target_frequency_cmd)
            got_data = await d.wait_for_data(timeout_s=1.0)

            if not got_data:
                # Retry with the previous app35 sequence as fallback.
                await d.force_stream_rearm(freq_cmd=self.target_frequency_cmd)
                got_data = await d.wait_for_data(timeout_s=1.2)

            if got_data:
                started.append({"address": d.address, "name": d.name})
            else:
                # Do not keep stale streaming state when no ADC notifications arrive.
                try:
                    await d.stop_stream()
                except Exception:
                    pass
    ...
    async def _wait_for_any_packets(self, timeout_s: float = 2.5, poll_s: float = 0.05) -> bool:
        ...
        while time.time() < end:
            for d in self.devices.values():
                if d.connected and (d.packet_count > 0 or d.raw_notify_count > 0):
                    return True
            await asyncio.sleep(poll_s)
        return False
                ...
                device_rows.append(
                    {
                        ...
                        "raw_notify_count": d.raw_notify_count,
                        "bad_packet_count": d.bad_packet_count,
                        "last_packet_size": d.last_packet_size,
...
def start_combined(capture_ble: bool = True):
    ...
    if capture_ble:
        try:
            ...
            raise RuntimeError(f"BLE start failed. Camera recording rolled back: {e}")

    start_recording_all()
    if capture_ble and current_recording_dir is not None:
