...
# ==================== App Globals ====================
app = Flask(__name__, template_folder="templates/new")
CORS(
    app,
    origins=[
...
_QUALITY_FPS_WINDOW = 60
_frame_ts_history = {k: deque(maxlen=_QUALITY_FPS_WINDOW) for k in CAMERA_SOURCES}
_last_sharpness_time = {k: 0.0 for k in CAMERA_SOURCES}
stream_quality = {
    k: {
        "actual_fps": None,
        "frame_drops": 0,
    ...
    for k in CAMERA_SOURCES
}
stream_quality_locks = {k: threading.Lock() for k in CAMERA_SOURCES}
...
def run_sync_on_dir(recording_dir: Path):
    ...
    if len(starts) < len(CAMERA_SOURCES):
        print("[sync] Missing start times. Skipping sync.")
        return
...
def _update_stream_quality(cam_key: str, frame, now: float):
    history = _frame_ts_history[cam_key]

    # Frame drop: gap between this frame and previous > 1.5× expected interval
    drops_delta = 0
    if history:
        gap = now - history[-1]
        if gap > 1.5 / TARGET_FPS_WRITE:
            drops_delta = 1
    history.append(now)

    # Rolling FPS over the last window of frames
    actual_fps = None
    if len(history) >= 2:
        elapsed = history[-1] - history[0]
        actual_fps = round((len(history) - 1) / elapsed, 1) if elapsed > 0 else None

    # Exposure metrics on grayscale
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    mean_brightness = round(float(cv2.mean(gray)[0]), 1)
    total = gray.shape[0] * gray.shape[1]
    underexposed_pct = round(float((gray <= 5).sum()) / total * 100, 2)
    overexposed_pct = round(float((gray >= 250).sum()) / total * 100, 2)

    # Sharpness via Laplacian variance — at most every 500ms
    sharpness = None
    if now - _last_sharpness_time[cam_key] >= 0.5:
        sharpness = round(float(cv2.Laplacian(gray, cv2.CV_64F).var()), 1)
        _last_sharpness_time[cam_key] = now

    with stream_quality_locks[cam_key]:
        q = stream_quality[cam_key]
        if actual_fps is not None:
            q["actual_fps"] = actual_fps
        q["frame_drops"] += drops_delta
        q["mean_brightness"] = mean_brightness
        q["overexposed_pct"] = overexposed_pct
        q["underexposed_pct"] = underexposed_pct
    ...
    while not stop_capture_evts[cam_key].is_set():
        ...
        while not stop_capture_evts[cam_key].is_set():
            ...
            with frame_locks[cam_key]:
                ...
                frame_ts[cam_key] = last_frame_wall

            _update_stream_quality(cam_key, frame, last_frame_wall)
...
def _draw_frame_centroid(frame):
    display = frame.copy()
    h, w = display.shape[:2]
    cx, cy = w // 2, h // 2
    arm = 24
    ...
    cv2.line(display, (cx - arm, cy), (cx + arm, cy), shadow, 3)
    cv2.line(display, (cx, cy - arm), (cx, cy + arm), shadow, 3)
    cv2.circle(display, (cx, cy), r + 1, shadow, -1)
    # crosshair
    cv2.line(display, (cx - arm, cy), (cx + arm, cy), color, 1)
    cv2.line(display, (cx, cy - arm), (cx, cy + arm), color, 1)
    cv2.circle(display, (cx, cy), r, color, -1)
    return display
...
def gen_frames(cam_key):
    while True:
        ...
        display = _draw_frame_centroid(frame)
        ok, buffer = cv2.imencode(".jpg", display, [cv2.IMWRITE_JPEG_QUALITY, current_jpeg_quality()])
        if not ok:
            time.sleep(0.01)
            continue
...
def _board_alignment(detection, frame_shape):
    """Return alignment info comparing board centroid to frame centroid.

    Expects detection to contain 'charuco_corners' (Nx1x2 float32 array) or
    'corners' as a list/array of corner positions. Falls back to bounding-box
    centroid of the board_crop rect if raw corners are unavailable.
    """
    h, w = frame_shape[:2]
    ...
    # Prefer raw sub-pixel corner positions if detect_and_segment exposes them.
    raw = detection.get("charuco_corners")
    if raw is not None:
        import numpy as np
        pts = np.array(raw).reshape(-1, 2)
        board_cx = float(pts[:, 0].mean())
        board_cy = float(pts[:, 1].mean())

    # Fallback: bounding rect of the board crop region.
    if board_cx is None:
        rect = detection.get("board_rect")  # (x, y, w, h) if provided
        if rect is not None:
            x, y, bw, bh = rect
            board_cx = x + bw / 2.0
            board_cy = y + bh / 2.0
    ...
    dx = board_cx - frame_cx
    dy = board_cy - frame_cy
    import math
    dist = math.hypot(dx, dy)
    max_dist = math.hypot(frame_cx, frame_cy)
    aligned = dist < 30  # within 30px of frame centre = well aligned

    return {
        "board_centroid_px": [round(board_cx, 1), round(board_cy, 1)],
        "frame_centroid_px": [round(frame_cx, 1), round(frame_cy, 1)],
        "offset_px": [round(dx, 1), round(dy, 1)],
        "distance_px": round(dist, 1),
        "distance_pct": round(100.0 * dist / max_dist, 1),
        "aligned": aligned,
    }
...
def focus_payload_for_camera(cam_key):
    ...
    alignment = _board_alignment(detection, frame.shape)
    return {
        "detected": True,
        "score": score,
        "label": label,
...
@app.route("/api/stream_quality", methods=["GET"])
def api_stream_quality():
    now = time.time()
    out = {}
    for cam_key in CAMERA_SOURCES:
        with stream_quality_locks[cam_key]:
            q = dict(stream_quality[cam_key])
        # Latency: age of the most recently captured frame
        ts = frame_ts.get(cam_key, 0.0)
        q["latency_ms"] = round((now - ts) * 1000, 1) if ts else None
        q["preview_healthy"] = _preview_is_healthy(cam_key)
        out[cam_key] = q
    return jsonify({"status": "success", "quality": out})
