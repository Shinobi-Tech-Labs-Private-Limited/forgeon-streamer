...
def stop_recording_all():
    ...
    if current_recording_dir and current_recording_dir.exists():
        # Block until synchronization is complete so API response contains the files
...
def _get_recording_files_info(rec_dir: Path) -> dict:
    ...
    Only returns synchronized files to the frontend, but originals remain on disk.
    ...
    for cam_key, semantic_name in CAMERA_NAME_MAPPING.items():
        ...
        if sync_file.exists():
            stat = sync_file.stat()
            # Map synced file to the base semantic name so frontend uses it by default
            files[semantic_name] = {
