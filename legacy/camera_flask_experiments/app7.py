...
def best_encoder_args():
    ...
    if (sys == "linux" or sys == "windows") and _ffmpeg_has_encoder("h264_nvenc"):
        return [
            ...
            "-b:v", "0",
            "-cq", "23",
            "-g", str(int(TARGET_FPS_WRITE)),  # GOP = 1s @ TARGET_FPS_WRITE
            "-profile:v", "high",
        ] + common_out
...
def _snap_output_dir() -> Path:
    """
    If recording: drop under current_recording_dir/snaps/snap_<ts>/
    Else:         sessions/.../snaps/snap_<ts>/
    """
...
def _snapshot_via_ffmpeg(cam_key: str, src_url: str, out_path: Path) -> bool:
    """
    Try to pull a single full-resolution JPEG directly from the RTSP stream.
...
def _snapshot_from_preview(cam_key: str, out_path: Path) -> bool:
    """
    Fallback: save the current preview frame (resized) if available.
    """
    with frame_locks[cam_key]:
        frame = frames.get(cam_key)
    if frame is None:
...
def capture_photos_all() -> dict:
    """
    Capture one image per camera. Prefer FFmpeg (full-res), fallback to preview.
    Returns a dict with saved file paths and method used.
    """
    ...
    for cam_key, src in CAMERA_SOURCES.items():
        ...
        method = "ffmpeg_rtsp"
        ok = _snapshot_via_ffmpeg(cam_key, src, out_path)
        if not ok:
...
def capture_photos():
    """
    Click photos from all cameras (full-res if possible).
    Redirect back to index after capture.
    """
    try:
        manifest = capture_photos_all()
        # Optionally, you could flash/JSON — keeping redirect to keep UI simple
        return redirect(url_for("index"))
    except Exception as e:
