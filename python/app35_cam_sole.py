...
def _build_remote_camera_command(cam_key: str) -> str:
    ...
    return (
        ...
        "fi; "
        f"pkill -f {shlex.quote('v4l2rtspserver.*-u ' + stream_name)} >/dev/null 2>&1 || true; "
...
def _run_remote_camera_command(cam_key: str, command: str):
    cfg = CAMERA_BOOTSTRAP[cam_key]
    target = f"{CAMERA_SSH_USER}@{cfg['host']}"
    ssh_cmd = [
        "ssh",
...
STREAMER_VERSION = "1.0.0"
_start_time = time.time()


@app.route("/api/streamer/health")
def streamer_health():
    """Lightweight health endpoint for frontend auto-detection."""
    cameras = {}
    for cam_key in CAMERA_SOURCES:
        health = _camera_health_payload(cam_key)
        cameras[cam_key] = {
            "name": health.get("name", cam_key),
            "connected": bool(health.get("camera_connected")),
            "healthy": bool(health.get("preview_healthy")),
            "state": health.get("state", "unknown"),
        }

    online_count = sum(1 for c in cameras.values() if c["healthy"])
    return jsonify(
        {
            "status": "ok",
